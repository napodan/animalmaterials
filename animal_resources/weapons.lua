...
mobf_assert_backtrace(AR_FIREBALL_ENTITY == nil)
...
mobf_assert_backtrace(AR_PLASMABALL_ENTITY == nil)
...
mobf_assert_backtrace(AR_ARROW_ENTITY == nil)
...
AR_ARROW_ENTITY.on_step = function(self, dtime)
	...
	if self.lastpos.x~=nil then
		if node.name ~= "air" then
			minetest.add_item(self.lastpos, 'animal_resources:arrow')
			self.object:remove()
		end
	end
...
function ar_init_weapons()
	minetest.register_entity(":animal_resources:fireball_entity", AR_FIREBALL_ENTITY)
	minetest.register_entity(":animal_resources:plasmaball_entity", AR_PLASMABALL_ENTITY)
	minetest.register_entity(":animal_resources:arrow_entity", AR_ARROW_ENTITY)
end


-- moved due to doxygen problems
minetest.register_node("animal_resources:arrow_box", {
	drawtype = "nodebox",
	node_box = {
		type = "fixed",
		fixed = {
...
if mobf_contains(mods,"throwing") then
	...
	minetest.register_alias("animal_resources:arrow", "throwing:arrow")
end
